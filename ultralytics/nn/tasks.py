...
from ultralytics.nn.Addmodels import (CoordAtt,CBAM,ECA,GAM,EMA)
...
from ultralytics.nn.Addmodels.Bi_FPN import *
from ultralytics.nn.backbone.ShuffleNetV2 import *
try:
    import thop
except ImportError:
    thop = None
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8sgai.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if c2 != nc:  # if not output
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        elif m in {CoordAtt,ECA,GAM,CBAM}:
            c2=ch[f]
            args=[c2,*args]
        elif m is AIFI:
            args = [ch[f], *args]
